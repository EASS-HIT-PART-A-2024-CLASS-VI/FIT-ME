...
def task_management_page():
    ...
    if response.status_code == 200:
        ...
        if tasks:
            for task in tasks:
                st.text(f"{task['first_name']} {task['last_name']} - {task['description']} Phone: {task['phone_number']}")
        else:
            st.info("No tasks available.")
    else:
        st.error("Failed to fetch tasks.")

    # Section 2: Add a new task
    ...
    # Section 3: Delete a task
    st.subheader("Delete a Task")
    delete_phone_number = st.text_input("Enter Phone Number of the Task to Delete", key="task_delete_phone_number")
